...
//Multiple threads covariance matrix
func paraCov(data *mat64.Dense, goro int) (covmat *mat64.Dense, err error) {
	...
	//init structure for threads job splitting
	element := coor{}
	var iArr [100]int
	var jArr [100]int
	k := 0

	//converting matrix op coor i,j into k
	for i := 0; i < nSets; i++ {
		for j := i; j < nSets; j++ {
			if k <= 99 {
				iArr[k] = i
	...
	//store i,j pairs into element and then c
	element.SetI(iArr)
	element.SetJ(jArr)
	c = append(c, element)

	//init array slice means and dense matrix covmat
